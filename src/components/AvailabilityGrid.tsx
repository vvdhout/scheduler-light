...
import { nowMin, overlaps } from '../lib/model';
...
export function AvailabilityGrid({ days, cells, onChange, busy = [], booked = [] }: Props) {
  const [dayIdx, setDayIdx] = useState(0);
  const [allHours, setAllHours] = useState(false);
  const paint = useRef<{ add: boolean; touched: Set<number> } | null>(null);
  const now = nowMin();
